...
| Network access | Gateway-enforced `network_policies` | REST read-only `webfetch` rules when port, path, and executable constraints are representable | Not represented in v1 because a complete profile requires deployment-specific proxy configuration | Not represented without an explicit runtime baseline |

OpenShell output is schema version 1 YAML. Filesystem paths must be absolute, root write access is rejected, Landlock is fixed to `hard_requirement`, workdir inclusion is fixed to `false`, the process cannot run as root, and every rendered endpoint uses `enforcement: enforce`. The fail-closed Landlock and workdir settings are not configurable. REST, WebSocket, and GraphQL access presets are supported. MCP and JSON-RPC require explicit method or tool rules that the v1 catalog does not yet model, so compilation rejects them.
