...
The service does not log Cedar source, catalogs, decisions, or generated artifacts. Network-facing service startup requires TLS 1.3 with a server certificate and mandatory verification against an operator-supplied client CA. That CA is the workload-identity trust boundary: deployments must issue client certificates only to workloads allowed to materialize policy and must rotate or revoke them through their certificate authority. Plain HTTP is an explicit development mode restricted to a numeric loopback address; it is not a network deployment profile. Rate limiting and request-metadata redaction remain deployment-edge responsibilities, and the built-in body limit protects memory rather than replacing those controls.
