...
A release is created from a signed or reviewed `vMAJOR.MINOR.PATCH` tag whose value matches `rosetta.Version` and the changelog. Release binaries and containers use the patched Go toolchain declared in `go.mod`; do not lower that floor without a vulnerability review. Pull requests run GoReleaser configuration and snapshot builds plus a multi-platform container build before publication is possible. The release workflow repeats the full formatting, test, race, vet, and build gate, builds platform archives with GoReleaser, and publishes checksums, SBOMs, and GitHub build-provenance attestations. The service image is published to GitHub Container Registry only after that source and binary release job succeeds. The Go SDK is distributed by the same Git tag through the Go module ecosystem.
...
Breaking changes require a new major version and a migration guide. Minor and patch releases must remain backward compatible and must not broaden generated access.
